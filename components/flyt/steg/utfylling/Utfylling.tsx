...
import { BodyLong, BodyShort, ErrorSummary, Heading, List, ReadMore, VStack } from '@navikt/ds-react';
        ...
        <VStack gap={'space-32'}>
          <VStack gap={'space-16'}>
            ...
            <ReadMore header={t('client.steg.utfylling.hvordanSkalJegFøreTimer.title')}>
              <BodyShort>
                {t.rich('client.steg.utfylling.hvordanSkalJegFøreTimer.content', { br: () => <br /> })}
              </BodyShort>
            </ReadMore>
          </VStack>
...
export function manglerTimerPåArbeid(dager: Dag[], harSvartJaPåArbeid: boolean): boolean {
  ...
  return dager.filter((dag) => dag.timer && Number(replaceCommasWithDots(dag.timer))).length === 0;
}
